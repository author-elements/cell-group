...
var AuthorCellGroupElement = (function () {
  ...
  (function () {
    ...
    if (missingDependencies.length > 0) {
      console.error("[ERROR] <author-cell-group> Required dependenc".concat(missingDependencies.length !== 1 ? 'ies' : 'y', " not found: ").concat(missingDependencies.map(function (d) {
        return "<".concat(d, ">");
      ...
      missingDependencies.forEach(function (dep, i) {
        return console.info("".concat(i + 1, ". <").concat(dep, "> is available at ").concat('https://github.com/author-elements/cell-group'.replace('cell-group', dep.replace('author-', ''))));
      });
    }
  })();

  var AuthorCellGroupElement =
  /*#__PURE__*/
  function (_AuthorBaseElement) {
    _inherits(AuthorCellGroupElement, _AuthorBaseElement);

    function AuthorCellGroupElement() {
      var _this;

      _classCallCheck(this, AuthorCellGroupElement);

      _this = _possibleConstructorReturn(this, _getPrototypeOf(AuthorCellGroupElement).call(this, "<template><style>@charset \"UTF-8\"; :host *,:host :after,:host :before{box-sizing:border-box}author-cell-group *,author-cell-group :after,author-cell-group :before{box-sizing:border-box}</style><slot></slot></template>"));
      ...
      _this.UTIL.defineProperties({
        defaultRole: {
          ...
          readonly: true,
          get: function get() {
            return sourceElement.localName === 'tr' ? 'row' : 'rowgroup';
          }
        },
        injected: {
          "private": true,
    ...
    _createClass(AuthorCellGroupElement, [{
      key: "inject",
...
//# sourceMappingURL=author-cell-group.es5.js.map
